...
class TrafficJunctionEnv(gym.Env):
    ...
    def init_args(self, parser):
        ...
        env.add_argument('--vision', type=int, default=0,
                         help="Vision of car")
        env.add_argument('--add_rate_min', type=float, default=0.1,
                         help="rate at which to add car (till curr. start)")
        env.add_argument('--add_rate_max', type=float, default=0.3,
                         help=" max rate at which to add car")
        env.add_argument('--curr_start', type=float, default=250,
                         help="start making harder after this many epochs [0]")
        env.add_argument('--curr_end', type=float, default=1250,
    ...
    def multi_agent_init(self, args):
        ...
        self.ncar = args.num_adversaries
    ...
    def step(self, action):
        ...
        # Sample the binomial input
        # print(np.shape(action))
        # action = [np.random.binomial(1, action[i][1], 1) for i in range(len(action))]
        ...
        return obs, reward, self.episode_over, self.stat['success']
