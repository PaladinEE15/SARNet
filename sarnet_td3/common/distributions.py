...
class PdType(object):
    ...
    def sample_placeholder(self, prepend_shape, name=None, traj=False):
        if traj:
            return tf.compat.v1.placeholder(dtype=self.sample_dtype(), shape=prepend_shape + prepend_shape + self.sample_shape(), name=name)
        else:
...
class OneHotPdType(PdType):
    def __init__(self, ncat):
        self.ncat = ncat
    def pdclass(self):
        return OneHotApproximationPd
    def param_shape(self):
        return [self.ncat]
    def sample_shape(self):
        return [self.ncat]
...
class OneHotApproximationPd(Pd):
    def __init__(self, logits):
        self.logits = logits
    def flatparam(self):
        return self.logits
    ...
    def sample(self, noise=True):
        ...
        samples = tf.reshape(samples, tf.shape(self.logits))
        return samples
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
...
def make_pdtype(ac_space, env_type=None):
    ...
    if isinstance(ac_space, spaces.Box):
        ...
        # if env_type == "ic3net":
            # return OneHotPdType(ac_space.n)
        # else:
