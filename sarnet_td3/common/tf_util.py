...
class BatchInput(PlacholderTfInput):
    def __init__(self, shape, dtype=tf.float32, name=None, traj=False):
...
def minimize_and_clip(optimizer, objective, var_list, clip_val=10):
    ...
    if clip_val is None:
        ...
        # nagents = tf.constant(nagents, dtype=tf.float32)
        gradients = optimizer.compute_gradients(objective, var_list=var_list)
        # print(gradients)
        for i, (grad, var) in enumerate(gradients):
            if grad is not None:
                # tgrad = tf.divide(tf.clip_by_norm(grad, clip_val), nagents)
...
def make_interactive_session(args):
    """Returns a session that will use <num_cpu> CPU's only"""
    # tf_config = tf.ConfigProto(
    #     inter_op_parallelism_threads=num_cpu,
    #     intra_op_parallelism_threads=num_cpu)
...
def inter_gpu_session(args):
    return make_interactive_session(args)
