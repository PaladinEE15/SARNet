...
class CommActorNetwork():
    ...
    def _gru(self, reuse):
        ...
        return tf.nn.rnn_cell.GRUCell(num_units=self.args.gru_units, reuse=reuse)

    def _lstm(self, reuse):
        return tf.nn.rnn_cell.LSTMCell(num_units=self.args.gru_units, reuse=reuse)
    ...
    def obs_encoder(self, x, state, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if self.args.recurrent:
                ...
                out, state = tf.nn.dynamic_rnn(enc, out, initial_state=state, time_major=False, scope="rnn_encoder")
            else:
    ...
    def compute_attn_dotprod(self, query, key_n, value_n, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            _obs_act_in = layers.batch_norm(message, decay=self.args.bnDecay, center=self.args.bnCenter,
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if self.args.SARplusIC3:
                ...
                for i in range(n_start, n_end):
                    _gating_comm = self.ic3_gating_com(h_n_in[i], scope="sarplusic3_gate", reuse=tf.AUTO_REUSE)
            ...
            for i in range(n_start, n_end):
                ...
                if self.args.pre_encoder:
                    obs_enc_in_i = layers.fully_connected(obs_enc_in_i, num_outputs=self.args.gru_units, activation_fn=tf.nn.relu, scope="sarnet_pre_enc", reuse=tf.AUTO_REUSE)
                ...
                if self.args.SARplusIC3:
                    ...
                    message_t1_i = self.commnet_com(saric3_comm_j, n_end - n_start - 1, scope="sarplusic3_mean", reuse=tf.AUTO_REUSE)
                ...
                if self.args.encoder_model == "LSTM":
                    obs_enc_state_i = tf.nn.rnn_cell.LSTMStateTuple(c_n_in[i], h_n_in[i])
                else:  # GRU
                ...
                _key_proj, _value_proj = self.get_key_val_vec(_enc_out, msg_n_in[i], scope="sarnet_KV_Projs", reuse=tf.AUTO_REUSE)
                _key_enc.append(_key_proj)
                _value_enc.append(_value_proj)
            # Generate nhead-queries only from action agent
            _query_enc = self.get_query_vec(obs_enc_out_n[p_index - n_start], scope="sarnet_QProj", reuse=tf.AUTO_REUSE)
            ...
            if self.args.bNorm_state:
                _obs_act_in = layers.batch_norm(_obs_act_in, decay=self.args.bnDecay, center=self.args.bnCenter,
                                             scale=self.args.bnScale, is_training=self.is_train, updates_collections=None, scope="batch_norm_obs", reuse=tf.AUTO_REUSE)

            _enc_input_2 = tf.concat([_obs_act_in, memory_state], axis=-1)
            if self.args.TwoLayerEncodeSarnet:
                _enc_input_2 = self.action_enc(_enc_input_2, scope="sarnet_act_enc", reuse=tf.AUTO_REUSE)
            action = self.action_pred(_enc_input_2, num_outputs, scope="sarnet_act", reuse=tf.AUTO_REUSE)
    ...
    def tarmac(self, x_n, num_outputs, p_index, n, n_start, n_end, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if self.args.TARplusIC3:
                _gating_comm_n = []
                for i in range(n_start, n_end):
                    _gating_comm = self.ic3_gating_com(h_n_in[i], scope="tarplusic3_gate", reuse=tf.AUTO_REUSE)
            ...
            for i in range(n_start, n_end):
                ...
                if self.args.TARplusIC3:
                    ...
                    message_t1_i = self.commnet_com(taric3_comm_j, n_end - n_start - 1, scope="tarplusic3_mean", reuse=tf.AUTO_REUSE)
                ...
                _key_proj, _value_proj = self.get_key_val_vec(_enc_out, msg_n_in, scope="tarmac_keyval", reuse=tf.AUTO_REUSE, comm_type = "tarmac")
                _key_enc.append(_key_proj)
                _value_enc.append(_value_proj)
            ...
            _query_enc = self.get_query_vec(obs_enc_out_n[p_index - n_start], scope="tarmac_query", reuse=tf.AUTO_REUSE, comm_type="tarmac")

            # Generate attention and message, attention only used for benchmarking results
            message_t1, attn = self.compute_attn_dotprod(_query_enc, _key_enc, _value_enc, scope="tarmac_attn", reuse=tf.AUTO_REUSE)

            obs_enc_p_idx = obs_enc_out_n[p_index - n_start]

            action = self.action_pred(obs_enc_p_idx, num_outputs, scope="tarmac_act", reuse=tf.AUTO_REUSE)
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for i in range(n_start, n_end):
                ...
                if p_index is not i + n_start:
                    _gating_comm = self.ic3_gating_com(h_n_in[i], scope="ic3_gate", reuse=tf.AUTO_REUSE)
                    _gating_comm_n.append(_gating_comm)

            # Average all scaled hidden states from the gating mechanism, received from the other agents
            message_t1_n = self.commnet_com(_gating_comm_n, n_end - n_start - 1, scope="mean_ic3net", reuse=tf.AUTO_REUSE)
            ...
            if self.args.pre_encoder:
                obs_enc_in_i = layers.fully_connected(obs_enc_in_i, num_outputs=self.args.gru_units,
                                                    activation_fn=tf.nn.relu, scope="ic3_pre_enc", reuse=tf.AUTO_REUSE)

            obs_enc_in_i = tf.concat([obs_enc_in_i, message_t1_n], axis=-1)

            # Encode observation with a recurrent model
            if self.args.encoder_model == "LSTM":
                obs_enc_state_i = tf.nn.rnn_cell.LSTMStateTuple(c_n_in[p_index - n_start], h_n_in[p_index - n_start])
            else:  # GRU
                obs_enc_state_i = h_n_in[p_index - n_start]

            obs_enc_out_pidx, obs_enc_state_pidx = self.obs_encoder(obs_enc_in_i, obs_enc_state_i, scope="ic3net_gru_enc", reuse=tf.AUTO_REUSE)
            obs_enc_out_pidx = tf.squeeze(obs_enc_out_pidx, axis=-2)

            action = self.action_pred(obs_enc_out_pidx, num_outputs, scope="ic3net_act", reuse=tf.AUTO_REUSE)
    ...
    def commnet(self, x_n, num_outputs, p_index, n, n_start, n_end, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            message_t1_n = self.commnet_com(_message_comm_j, n_end - n_start - 1, scope="mean_commnet", reuse=tf.AUTO_REUSE)

            # Encode observation
            obs_enc_in_pidx = obs_n_in[p_index - n_start]
            if self.args.pre_encoder:
                obs_enc_in_pidx = layers.fully_connected(obs_enc_in_pidx, num_outputs=self.args.gru_units, activation_fn=tf.nn.relu, scope="comm_pre_enc", reuse=tf.AUTO_REUSE)
            ...
            _enc_out, _enc_state = self.obs_encoder(obs_enc_in_pidx, obs_enc_state_i, scope="commnet_enc", reuse=tf.AUTO_REUSE)
            _enc_out = tf.squeeze(_enc_out, axis=-2)

            action = self.action_pred(_enc_out, num_outputs, scope="comm_act", reuse=tf.AUTO_REUSE)
    ...
    def ddpg(self, x_n, num_outputs, p_index, n, n_start, n_end, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if self.args.encoder_model == "LSTM":
                ...
                obs_enc_state_i = h_n_in[p_index - n_start]
            _enc_out, _enc_state = self.obs_encoder(_enc_input, obs_enc_state_i, scope="ddpg_enc", reuse=tf.AUTO_REUSE)
            _enc_out = tf.squeeze(_enc_out, axis=-2)

            action = self.action_pred(_enc_out, num_outputs, scope="ddpg_act", reuse=tf.AUTO_REUSE)
