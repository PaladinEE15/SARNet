...
class CommActorNetworkVPG():
    ...
    def _gru(self, reuse, num_outputs=None):
    ...
    def _lstm(self, reuse, num_outputs=None):
    ...
    def get_key_val_vec(self, x, scope, reuse, comm_type="sarnet"):
        with tf.variable_scope(scope, reuse=reuse):
            x_val = x
            # Input [batch, dim]
            if self.args.nheads > 1 and not comm_type == "tarmac":
    ...
    def sarnet(self, x_n, num_outputs, p_index, n, n_start, n_end, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            outputs_value = tf.TensorArray(dtype=tf.float32, size=0, dynamic_size=True)

            # Control the size of the loop with the time dimension of the observation space
            time_steps = tf.shape(obs_n_in[p_index - n_start])[0]
            def condition(stp, *args):
                return stp < time_steps

            # lambda stp, x: stp < time_steps

            def body(stp, h_n_t, c_n_t, msg_n_t, outputs_, outputs_val):
                ...
                for i in range(n_start, n_end):
                    ...
                    if i is p_index:
                        _enc_out_p_index = _enc_out

                    _key_proj, _value_proj = self.get_key_val_vec(_enc_out, scope="sarnet_KV_Projs", reuse=tf.AUTO_REUSE)
                    _key_enc.append(_key_proj)
                    _value_enc.append(_value_proj)

                _query_enc = self.get_query_vec(_enc_out_p_index, scope="sarnet_QProj", reuse=tf.AUTO_REUSE)

                # Reasoning Cell
                self.sarnet_cell = self._sarnet_comm(reuse=tf.AUTO_REUSE)
                sarnet_input = (_query_enc, _key_enc, _value_enc)

                attn_pidx, msg_n_t[p_index - n_start] = self.sarnet_cell(sarnet_input, msg_n_t[p_index - n_start])
                # Merge new reasoned message and observation encoding
                _obs_act_in = _enc_out_p_index
                ...
                act_in_pred = tf.concat([_obs_act_in, msg_n_t[p_index - n_start]], axis=-1)
                ...
                value = self.value_pred(act_in_pred, 1, scope="sarnet_value", reuse=tf.AUTO_REUSE)
                outputs_val = outputs_val.write(stp, value)

                return stp + 1, h_n_t, c_n_t, msg_n_t, outputs_, outputs_val

            _, h_n_out, c_n_out, msg_n_out, out, out_val = tf.while_loop(condition, body, loop_vars=[0, h_n_in, c_n_in, msg_n_in, outputs_act, outputs_value], parallel_iterations=1)

            out = out.stack()  # This will throw out a [time, out_dim_list]
            out_val = out_val.stack()

            action_traj = out
            msg_pidx_out = msg_n_out[p_index - n_start]

        return action_traj, (h_n_out[p_index - n_start], c_n_out[p_index - n_start]), msg_pidx_out, attn_pidx, out_val
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            def body(stp, h_n_t, c_n_t, msg_n_t, outputs_, outputs_val):
                ...
                for i in range(n_start, n_end):
                    ...
                    msg_n_t[i], _ = self.compute_attn_dotprod(_query_enc[i], _key_enc, _value_enc, scope="tarmac_attn", reuse=tf.AUTO_REUSE)

                action = self.action_pred(obs_enc_out_n[p_index - n_start], num_outputs, scope="tarmac_act", reuse=tf.AUTO_REUSE)
                value = self.value_pred(obs_enc_out_n[p_index - n_start], 1, scope="ic3net_value", reuse=tf.AUTO_REUSE)
            ...
            # Run the While Loop
            _, h_n_out, c_n_out, msg_n_out, out, out_val = tf.while_loop(condition, body, loop_vars=[tf.Variable(0), h_n_in, c_n_in, msg_n_in, outputs_, outputs_value], parallel_iterations=1)
            out = out.stack()  # This will throw out a [time, out_dim_list]
            out_val = out_val.stack()
            ...
            return action_traj, (h_n_out[p_index - n_start], c_n_out[p_index - n_start]), msg_pidx_out, msg_pidx_out, out_val
    ...
    def ic3net(self, x_n, num_outputs, p_index, n, n_start, n_end, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            # Run the While Loop
            _, h_n_out, c_n_out, msg_n_out, out, out_val = tf.while_loop(condition, body, loop_vars=[0, h_n_in, c_n_in, msg_n_in, outputs_, outputs_value], parallel_iterations=1)
            out = out.stack()  # This will throw out a [time, out_dim_list]
            out_val = out_val.stack()
    ...
    def commnet(self, x_n, num_outputs, p_index, n, n_start, n_end, scope, reuse):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            def body(stp, h_n_t, c_n_t, msg_n_t, outputs_, outputs_val):
                ...
                value = self.value_pred(obs_enc_out_n[p_index - n_start], 1, scope="commnet_value", reuse=tf.AUTO_REUSE)
                outputs_val = outputs_val.write(stp, value)
