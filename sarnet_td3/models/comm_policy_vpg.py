...
class CommActorNetworkVPG():
    ...
    def _lstm(self, reuse, proj=False, num_outputs=None):
        if proj:
            ...
            return tf.contrib.rnn.OutputProjectionWrapper(lstm_cell, num_outputs, reuse=reuse)
        else:
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            ...
            def body(stp, h_n_t, c_n_t, msg_n_t, outputs_, outputs_val):
                ...
                for i in range(n_start, n_end):
                    ...
                    _key_proj, _value_proj = self.get_key_val_vec(_enc_out, scope="sarnet_KV_Projs", reuse=tf.compat.v1.AUTO_REUSE)
                    _key_enc.append(_key_proj)
                    _value_enc.append(_value_proj)

                _query_enc = self.get_query_vec(_enc_out_p_index, scope="sarnet_QProj", reuse=tf.compat.v1.AUTO_REUSE)
                ...
                value = self.value_pred(act_in_pred, 1, scope="sarnet_value", reuse=tf.compat.v1.AUTO_REUSE)
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            ...
            def body(stp, h_n_t, c_n_t, msg_n_t, outputs_, outputs_val):
                ...
                for i in range(n_start, n_end):
                    ...
                    msg_n_t[i], _ = self.compute_attn_dotprod(_query_enc[i], _key_enc, _value_enc, scope="tarmac_attn", reuse=tf.compat.v1.AUTO_REUSE)

                action = self.action_pred(obs_enc_out_n[p_index - n_start], num_outputs, scope="tarmac_act", reuse=tf.compat.v1.AUTO_REUSE)
                value = self.value_pred(obs_enc_out_n[p_index - n_start], 1, scope="ic3net_value", reuse=tf.compat.v1.AUTO_REUSE)
    ...
    def commnet(self, x_n, num_outputs, p_index, n, n_start, n_end, scope, reuse):
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            ...
            def body(stp, h_n_t, c_n_t, msg_n_t, outputs_, outputs_val):
                ...
                value = self.value_pred(obs_enc_out_n[p_index - n_start], 1, scope="commnet_value", reuse=tf.compat.v1.AUTO_REUSE)
                outputs_val = outputs_val.write(stp, value)
